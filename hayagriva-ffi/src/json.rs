...
struct StyleOut<'a> {
    ...
    citation_format: Option<&'a str>,
}

pub fn list_styles(styles: &[(String, String, Option<String>, Option<String>)]) -> String {
    let out: Vec<StyleOut> = styles
        .iter()
        .map(|(key, display_name, default_locale, citation_format)| StyleOut {
            key,
            display_name,
            default_locale: default_locale.as_deref(),
            citation_format: citation_format.as_deref(),
        })
        .collect();
    miniserde::json::to_string(&out)
}
